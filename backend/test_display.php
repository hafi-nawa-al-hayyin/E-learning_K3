...
echo "Testing tampilan riwayat simulasi...\n";
...
if ($result_simulasi) {
    ...
    while($s = mysqli_fetch_assoc($result_simulasi)) {
        // Simulasi tampilan yang sama dengan index.php
        $color = ($s['status_kelulusan'] == "LULUS") ? "#28a745" : "#dc3545";
        echo "Nama: " . $s['nama_lengkap'] . ", Risiko: " . $s['jenis_risiko'] . ", Skor: " . $s['skor'] . ", Status: " . $s['status_kelulusan'] . "\n";
    }
} else {
    echo "Error query simulasi: " . mysqli_error($conn) . "\n";
}

echo "\nTesting tampilan decision logs...\n";
...
if ($result_logs) {
    ...
    while($l = mysqli_fetch_assoc($result_logs)) {
        // Simulasi tampilan yang sama dengan index.php
        $color = ($l['status_kelulusan'] == "LULUS") ? "#28a745" : "#dc3545";
        echo "Nama: " . $l['nama_lengkap'] . ", Risiko: " . $l['jenis_risiko'] . ", Tindakan: " . substr($l['tindakan_dipilih'], 0, 30) . "...\n";
        echo "Skor: " . $l['skor'] . ", Status: " . $l['status_kelulusan'] . ", Waktu: " . date('d/m/Y H:i', strtotime($l['created_at'])) . "\n\n";
    }
} else {
    echo "Error query decision_logs: " . mysqli_error($conn) . "\n";
}

echo "Test tampilan selesai! Tidak ada warning undefined array key.\n";
?>